import { PlayerError, type ErrorCode } from './errors';
...
export class ChannelSocket {
  ...
  attach(channel: RTCDataChannel) {
    if (this.closed) { channel.close(); throw new PlayerError('CHANNEL_CLOSED'); }
    if (this.channel || channel.label !== 'sendspin' || !channel.ordered || channel.readyState !== 'open') {
      channel.close(); throw new PlayerError('CHANNEL_INVALID');
    }
